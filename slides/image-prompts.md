...
### S18 — (Appendix) The NixOS path — *The reproducible seed vault* — `section-18-reproducible-seed-vault.png`

A crystalline **seed vault** at the edge of the links: from one sealed frosted-glass
seed, several **identical golden greens** unfold in perfect copies, each
indistinguishable from the last — pure reproducibility. A faint *blueprint-ghost*
overlay marks it clearly as **planned, not yet grown**. Cool, pristine, slightly
dreamlike. *(Designed — compose with a subtle unfinished/translucent quality.)*

### S19 — (Appendix) Repo tour — *The estate map* — `section-19-estate-map.png`

A hand-drawn **estate map** of the whole links pinned to the clubhouse wall —
labelled paddocks for each part of the grounds (deploy fields, module sheds, packer
workshop, policy gatehouse, evidence vault, test range), drawn as a neat top-down
cartographer's plan with little icons. Kaddy points a club at it like a tour guide.
Warm, inviting "here's the whole estate at a glance" energy.

### S20 — (Appendix) Quickstart & tools — *Kaddy's kit bag* — `section-20-kaddys-kit-bag.png`

The enchanted golf bag tipped open on the first tee, its clubs neatly fanned out and
**labelled as tools of the trade** (a kind-shaped wedge, a helm-shaped putter, a
tofu-block, a packer flask) ready to grab; a short numbered chalk path leads from the
bag to a flag reading only with an open gate. Crisp "grab these, follow the path,
you're playing in five minutes" energy. Keep left third calm.

### S21 — (Appendix) Solved different ways — *Three routes to the same green* — `section-21-three-routes.png`

Three distinct fairways converge on **one single flagged green**: a modest tidy **VM
cottage path** (the literal brief), a richer terraced **K8s path** with more
greenkeepers, and a pristine translucent **Nix path** drawn as a designed/ghostly
future route. Kaddy stands at the fork, happy, having clearly walked all three. "Same
destination, deliberate choices" energy.
