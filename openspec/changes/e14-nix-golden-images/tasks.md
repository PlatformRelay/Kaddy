...
- [x] **Boot-to-serve GREEN (2026-07-19 loop3).** Root cause found + fixed at source: the golden `Caddyfile`'s global `metrics { per_host }` block needs caddy ≥ 2.9, but nixos-24.11 ships caddy 2.8.4 → `caddy run` exits before binding :80 → serves nothing (the "config bug" candidate above). Fix `485efa0`: pin `services.caddy.package` to caddy 2.10.x via a `nixpkgs-caddy = nixos-25.05` flake input. Live re-prove from the refreshed `.gz` + new import `2bfb228a-…`: `GET /` 200 + `/healthz` "ok" + `:2019/metrics` 200 (158 `caddy_` series), torn down clean. Evidence: `evidence/live/e14-nix-deploy-2026-07-19.md`.
- [x] Prometheus-operator `ScrapeConfig` targeting `<ip>:2019/metrics` (GSK `monitoring` ns) → **`up=1` PROVEN (2026-07-19 loop3)** for `job="caddy-nix-e14"` — same live cycle, via `POST_SERVE_HOOK`/`hack/e14-s03-scrape-hook.sh`; ScrapeConfig + VM torn down clean. Same `job="caddy"` contract E13-S05 proved for the Ubuntu target, now with the Nix image.
...
- [x] Live register→deploy→serve→scrape cycle **FULLY PROVEN (2026-07-19 loop3)**: register (S02) LIVE-DONE + deploy + serve (`GET /` 200, `/healthz`, 158 `caddy_` series) + Prometheus `up=1` — all torn down clean (0 orphans). Root cause was the caddy 2.8.4 version skew (fixed `485efa0`), not the boot contract. Evidence: `evidence/live/e14-nix-{marketplace,deploy}-2026-07-19.md`
