...
- [~] E6g-S03: Extend Composition for gridscale_server nginx — *offline-complete: `deploy/crossplane/composition-website-gridscale.yaml` composes Server (1 core/1 GiB nginx VM, cloud-init page+/metrics) + IPv4/Storage (public NIC via the external gridscale Public Network — the dead composed Network MR was dropped per D-039); variant-selected 2nd Composition (in-cluster path untouched). MRs validate vs sibling CRDs. **Provider proven to actuate real gridscale infra (S02 Network MR)**; the full Website-XR→gridscale_server VM provision still DEFERRED*
